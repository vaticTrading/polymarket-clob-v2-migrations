"""
6_market_buy_order.py — FOK market BUY at best ask → poll until filled.

EOA-based: pUSD held by EOA. Run 7_market_sell_order.py next to sell the YES tokens.
...
BUY_SPEND   = 2.0   # dollars to spend
...
def get_pusd() -> float:
    return pusd_contract.functions.balanceOf(eoa).call() / 1e6
...
if not asks:
    print("No asks in the book — can't market buy"); exit(1)

best_ask = float(asks[0]["price"])
balance  = get_pusd()

print(f"Best ask  : ${best_ask}  pUSD balance: ${balance:.4f}")
print(f"Buy spend : ${BUY_SPEND}\n")

if balance < BUY_SPEND + 0.5:
    print("⚠ Not enough pUSD"); exit(1)
...
for i in range(30):
    ...
    if buy_id not in ids:
        print("✓ Filled")
        break
    process_dot = "." if i < 29 else "\n⚠ Still open after 30 s"
    print(process_dot, end="", flush=True)
else:
    print()
