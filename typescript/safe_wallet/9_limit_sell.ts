/**
 * 8_limit_sell.ts — GTC limit SELL YES tokens (from split) 2 ticks above best ask → confirm → cancel.
 *
 * Safe wallet: signatureType=2, funderAddress=SAFE. YES tokens held by Safe.
 * Run 7_split.ts first to obtain YES tokens.
 *
 * Run:
 *   npm run 8:sell
 */
...
// ── Step 1: Init ClobClient V2 (Safe / signatureType=2) ──────────────────────
...
console.log("ClobClient V2 (Safe) initialized ✓");
...
// ── Step 3: Check YES token balance in Safe ───────────────────────────────────
...
if (yesShares < 0.01) {
  console.error("⚠ Not enough YES tokens in Safe — run 7_split.ts first.");
  process.exit(1);
}
...
if (asks.length === 0) {
  console.log("⚠ Ask book is empty — placing sell 2 ticks above mid or at 0.98");
}
