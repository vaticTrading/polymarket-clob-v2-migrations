"""
5_limit_buy_order.py — GTC BUY 2 ticks below best bid (Safe collateral) → cancel.

Run:
    python 5_limit_buy_order.py

Requires: PRIVATE_KEY, SAFE_ADDRESS, CLOB_V2_BASE_URL, POLYGON_RPC_URL in .env
pUSD collateral is locked in the Safe. signatureType=2.
"""
...
# ── pUSD balance comes from Safe (not EOA) ────────────────────────────────────
raw_bal = w3.eth.contract(address=PUSD, abi=ERC20_ABI).functions.balanceOf(SAFE).call()
balance = raw_bal / 1e6
if balance < 1:
    print("⚠ Not enough pUSD in Safe."); exit(1)

size = round((balance - 1) / price, 2)
print(f"Balance : ${balance:.4f} pUSD (Safe)")
