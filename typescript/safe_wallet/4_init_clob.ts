/**
 * 4_init_clob.ts — Init ClobClient V2 with Gnosis Safe (signatureType=2).
 *
 * signatureType=2 → Safe wallet mode: EOA signs but Safe is the funder.
 * funderAddress   → the Safe that holds pUSD collateral.
 *
 * Run:
 *   npm run 4:init
 */
...
// ── Step 1: Derive API key (EOA signs, but Safe is funder) ────────────────────

const tempClient = new ClobClient({
  host:          CLOB_HOST,
  chain:         137,
  signer:        walletClient as never,
  signatureType: 2,               // ← Gnosis Safe mode
  funderAddress: SAFE_ADDRESS,    // ← Safe holds the collateral
});
