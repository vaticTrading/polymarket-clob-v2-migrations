/**
 * 2_allowances.ts — Set pUSD + CTF allowances on the Safe for V2 contracts.
 *
 * The Safe (not the EOA) holds pUSD and CTF tokens, so all approvals must
 * go through Safe.execTransaction signed by the EOA.
 *
 * Required:
 *   ERC-20  pUSD → approve MAX to: CTF, CTF Exchange V2, NR Exchange V2,
 *                                   NR Adapter, CtfCollateralAdapter, NegRiskCtfCollateralAdapter,
 *                                   CollateralOfframp (for unwrapping pUSD → USDC.e)
 *   ERC-1155 CTF → setApprovalForAll to: same 5 (minus CTF itself)
 *
 * Run:
 *   npm run 2:allow
 */
...
const [pusdVals, ctfVals] = await Promise.all([
  Promise.all(erc20Targets.map(([, addr]) =>
    publicClient.readContract({ address: PUSD, abi: erc20Abi, functionName: "allowance", args: [SAFE_ADDRESS, addr] })
  )),
  Promise.all(erc1155Targets.map(([, addr]) =>
    publicClient.readContract({ address: CTF, abi: ERC1155_ABI, functionName: "isApprovedForAll", args: [SAFE_ADDRESS, addr] })
  )),
]);
...
// ── Step 2: Send missing approvals via Safe.execTransaction ───────────────────

let count = 0;

for (const [[, addr], val] of erc20Targets.map((t, i) => [t, pusdVals[i]] as const)) {
  if (val < maxUint256) {
    console.log(`\nApproving pUSD → ${addr} via Safe…`);
    const data = encodeFunctionData({ abi: erc20Abi, functionName: "approve", args: [addr, maxUint256] });
...
for (const [[, addr], val] of erc1155Targets.map((t, i) => [t, ctfVals[i]] as const)) {
  if (!val) {
    console.log(`\nApproving CTF → ${addr} via Safe…`);
    const data = encodeFunctionData({ abi: ERC1155_ABI, functionName: "setApprovalForAll", args: [addr, true] });
