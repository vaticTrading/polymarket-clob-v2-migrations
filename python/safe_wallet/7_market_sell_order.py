"""
7_market_sell_order.py — FOK market SELL of full YES token balance (Safe).

Safe-based: reads YES token balance from CTF (ERC-1155 balanceOf(SAFE, tokenId)),
sells full amount at best bid. Run 6_market_buy_order.py first to have YES tokens.
...
# ── YES token balance (ERC-1155, held by Safe) ────────────────────────────────
...
raw_pusd   = pusd_contract.functions.balanceOf(SAFE).call()
pusd_before = raw_pusd / 1e6

print(f"YES balance  : {yes_shares:.6f} shares (Safe)")
print(f"pUSD before  : ${pusd_before:.6f} (Safe)")
...
for i in range(10):
    time.sleep(3)
    raw_yes_after  = ctf_contract.functions.balanceOf(SAFE, int(token_id)).call()
    raw_pusd_after = pusd_contract.functions.balanceOf(SAFE).call()
